...
def run_sequential(args, logger):
    ...
    # 初始化runner对象，用于管理环境交互
    # 示例数据: args.runner = "episode"表示使用回合制runner
    runner = r_REGISTRY[args.runner](args=args, logger=logger)

    # Set up schemes and groups here
    # 设置环境信息和数据格式
    if 'particle' not in args.env and "cts_matrix_game" not in args.env and "mujoco_multi" not in args.env:
        env_info = runner.get_env_info() # 获取离散动作空间的环境信息  示例: {"n_agents": 8, "n_actions": 14, "state_shape": 120, "obs_shape": 80}
        print("===================================================================================")
        print("env_info:", env_info)
        print("===================================================================================")
        '''
        env_info: {'state_shape': 27, 'obs_shape': 36, 'n_actions': 9, 'n_agents': 3, 'episode_limit': 150, 'agent_features': ['health', 'energy/cooldown', 'rel_x', 'rel_y', 'shield'], 'enemy_features': ['health', 'rel_x', 'rel_y', 'shield']}
        '''
        ...
        groups = {
            "agents": args.n_agents   # 示例: {"agents": 8}
        }
        preprocess = {
        ...
        # 处理连续动作空间的环境
        env_info = runner.get_env_info()  # 示例: {"n_agents": 2, "n_actions": 5, "state_shape": 30, ...}
        args.n_agents = env_info["n_agents"]
        args.n_actions = env_info["n_actions"]
        args.state_shape = env_info["state_shape"]
        args.obs_shape = env_info["obs_shape"]
    ...
    # 初始化经验回放缓冲区
    # 示例: buffer_size=5000, episode_limit=200
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            for _ in range(n_test_runs):
                if getattr(args, "runner_scope", "episodic") == "episodic":
                    ...
                    raise Exception("Undefined runner scope!")
                rl_eval_reward += rl_tp_reward  # 累计奖励
                rl_battle_win_rate += tp_win  # 累计胜率
