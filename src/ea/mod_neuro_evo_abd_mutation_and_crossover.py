...
class SSNE:
    """稳态神经进化算法实现 (Steady-State Neuroevolution)"""
    ...
    def mutate_inplace(self, gene, agent_index, agent_level= False):
            ...
            for i, key in enumerate(model_params): #Mutate each param
                ...
                if len(W.shape) == 2: #Weights, no bias
                    ...
                    if random.random() < ssne_prob * self.args.mutation_prob:
                        num_mutations = fastrand.pcg32bounded(int(math.ceil(self.frac * num_weights)))  # Number of mutation instances
